classdef AnovaMU < nirs.modules.AbstractModule
    % Group-level Anova module employing the mass univariate approach
    % (i.e., separate model for each channel or ROI)
    %
    % By default produces the same results as if you extracted all
    % channels/ROIs and ran each through SPSS w/ Type III SS
    properties
    ...
    methods
        function obj = AnovaMU( prevJob )
            obj.name = 'Anova Model (mass univariate)';
            if nargin > 0
                obj.prevJob = prevJob;
            end
        end
        ...
        function G = runThis( obj, S )
            ...
            % Prepare output
            coefnames = lme1.anova.Term(:);
            ncoef = length(coefnames);
            G = nirs.core.ChannelFStats();
            G.probe = S(1).probe;
            G.description = ['ANOVA (univariate) Model: ' obj.formula];
            ...
            G.F = nan(nchan*ncoef,1);
            G.df1 = nan(nchan*ncoef,1);
            G.df2 = nan(nchan*ncoef,1);
            ...
            for chan = 1:nchan
                ...
                lme2 = fitlmematrix( X, resp(:,chan), Z, [], 'dummyVarCoding',obj.dummyCoding, ...
                    'FitMethod', 'ML', 'CovariancePattern', 'Isotropic', 'Weights', W);
                
                anova = lme2.anova('DFMethod',obj.DFMethod);
                
                outinds = (chan-1)*ncoef+1 : chan*ncoef;
                
                G.F(outinds) = anova.FStat;
                G.df1(outinds) = anova.DF1;
                G.df2(outinds) = anova.DF2;
