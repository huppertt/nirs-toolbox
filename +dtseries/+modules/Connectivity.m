        ...
        function connStats = runThis( obj, data )
            for i = 1:length(data)
                ...
                if(obj.divide_events)
                    ...
                    for idx=1:size(X,2);
                        if(~all(X(:,idx)==0))
                            ...
                            disp(['excluding stimulus: ' names{idx}]);
                        end
                    end
                ...
                for cIdx=1:length(mask)
                    tmp=data(i);
                    p=data(i).projectors';
                    lst=find(sum(abs(p),1)~=0);
                    ii=unique(tmp.mesh.link.type);
                    d=[];
                    for id=1:length(ii)
                        lst2=find(ismember(tmp.mesh.link.type,ii(id)));
                        d=[d tmp.data(:,lst2)*p(:,lst)];
                    end
                    tmp.data=d;
