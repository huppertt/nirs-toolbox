function [b,stats,lambda] = robustfit(X,y,Qn,Qp,lambda,wfun,tune,const,priorw,dowarn)
...
if (nargin<6 || isempty(wfun)), wfun = 'bisquare'; end
if nargin<7, tune = []; end
[wfun,tune] = statrobustwfun(wfun,tune);
if (nargin<8), const='off'; end
switch(const)
 case {'on' 1},  doconst = 1;
...
[b,stats,lambda] = statrobustfit(X,y,Qn,Qp,lambda,wfun,tune,[],doconst,priorw,dowarn);




function [b,stats,lambda] = statrobustfit(X,y,Qn,Qp,lambda,wfun,tune,wasnan,addconst,priorw,dowarn)
...
while((iter==0) || any(abs(b-b0) > D*max(abs(b),abs(b0))))
   ...
   b0 = b;
   [b(perm),lambda,stats] = wfit(y,X,w,perm,Qn,Qp,lambda);
end
...
% -----------------------------
function [b,lambda,stats] = wfit(y,x,w,perm,Qn,Qp,lambda)
...
for i=1:length(Qn)
    Qn{i}=diag(sw)*Qn{i}*diag(sw)';
end


[lambda,b,stats]=nirs.math.REML(yw,xw,[],Qn,Qp);
stats=stats.tstat;
