function data = simARNoise_NonStationary( probe, t, P, sigma, driftFreq, N_files )
    
    if (nargin < 6 || isempty(N_files)), N_files=1; end;
    if (nargin < 5 || isempty(driftFreq)), driftFreq = .008; end;
    ...
    Fs=1./mean(diff(t));
    [fa,fb]=butter(4,driftFreq*2/Fs);
    e2=1+.45*sin(2*pi*driftFreq*t+pi*filter(fa,fb,2*pi*randn(size(t))));
    e=e.*e2;
