function [data, truth] = simData( noise, stim, beta, channels, basis,SCnoise )
...
if(nargin<6 || isempty(SCnoise))
    SCnoise=[.2; .05];
end

if(size(SCnoise,1)~=2 & size(SCnoise,2)==2)
    SCnoise=SCnoise';
end

if(size(SCnoise,2)<stim.count)
    SCnoise=SCnoise*ones(1,stim.count);
end


GroupSingleTrialNoise=SCnoise(1,:);
ChannelLevelSingleTrialNoise=SCnoise(2,:);
...
for ii=1:length(keys)
    stim2(keys{ii}).amp=stim2(keys{ii}).amp+randn(size(stim2(keys{ii}).amp))*GroupSingleTrialNoise(ii);
end
...
if(~(iscellstr(link.type) && any(ismember(link.type,{'hbo','hbr'}))))
    ...
    for i = 1:size(channels, 1)
        ...
        keys=stim.keys;
        for ii=1:length(keys)
            stim3(keys{ii}).amp=stim3(keys{ii}).amp+randn(size(stim3(keys{ii}).amp))*ChannelLevelSingleTrialNoise(ii);
        end
        Xhbo = nirs.design.createDesignMatrix( stim3, data.time, basis, 'hbo' );
        Xhbr = nirs.design.createDesignMatrix( stim3, data.time, basis, 'hbr' );
