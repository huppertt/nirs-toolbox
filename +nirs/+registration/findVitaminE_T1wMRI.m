...
%vol2=medfilt3(double(vol.img),[5 5 5]);
c=ones(5,5,5);
%% 
c(3,3,3)=0;
vol2=convn(double(vol.img),c,'same');
...
for j=1:(numMarkers)
    connected2(ismember(connected,i(j)))=j;
    VitE.Name{j,1}='VitE';
    VitE.X(j,1)=nanmean(nodes(ismember(connected,i(j)),1));
    VitE.Y(j,1)=nanmean(nodes(ismember(connected,i(j)),2));
    VitE.Z(j,1)=nanmean(nodes(ismember(connected,i(j)),3));
    VitE.Type{j,1}='FID';
    VitE.Units{j,1}='mm';
end
...
if(showplot)
    ...
    hold on; for i=1:numMarkers; s(i)=scatter3(VitE.X(i),VitE.Y(i),VitE.Z(i)); end;
    set(s,'SizeData',3000)
