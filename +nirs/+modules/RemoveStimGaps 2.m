classdef RemoveStimGaps < nirs.modules.AbstractModule
    ...
    properties
        ...
        minISI =NaN;
        seperate_conditions=true;  % this will process each cond type seperately
    end
    ...
    methods
        ...
        function data = runThis( obj, data )
            ...
            for i = 1:numel(data)
                ...
                if(obj.seperate_conditions & length(nirs.getStimNames(data(i)))>1)
                    stim=data(i).stimulus;
                    
                    for ii=1:stim.count
                        data(i).stimulus=Dictionary;
                        data(i).stimulus(stim.keys{ii})=stim(stim.keys{ii});
                        data(i)=obj.runThis(data(i));
                        stim(stim.keys{ii})=data(i).stimulus(stim.keys{ii});
                    end
                    data(i).stimulus=stim;
