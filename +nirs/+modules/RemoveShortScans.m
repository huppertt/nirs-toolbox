...
% 
    properties
        mintime=30;  % time in seconds AND samples.  Usually seconds will be more strict.
    ...
    methods
        ...
        function data = runThis( obj, data )
            lstbad=[];
            for i = 1:numel(data)
              if((max(data(i).time)-min(data(i).time))<obj.mintime | length(data(i).time)<obj.mintime)
                  lstbad=[lstbad; i];
              end
