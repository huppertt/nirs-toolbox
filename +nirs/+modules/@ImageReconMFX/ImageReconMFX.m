    ...
    methods
        ...
        function G = runThis( obj,S )
            ...
            for i = 1:L.count
                ...
                Ltmp=[];
                for j=1:length(flds)
                    Ltmp=[Ltmp J.(flds{j})(:,LstInMask)*W(LstInMask,:)];
                    %                     LL{i,j}=J.(flds{j})*W;
            ...
            for id=1:nPriors
                ...
                for idx=1:length(Sfake)
                    ...
                    Sfake(idx).demographics('subject')= 'prior';
                    
                    Sfake(idx).probe.link=sortrows(Sfake(idx).probe.link,{'type'});
                    b=[];
                    C=[];
                    for i=1:length(Sfake(idx).conditions)
                       ...
                       for j=1:length(types)
                        ...
                        b =[b; q'*pp/m];
                        C = blkdiag(C,m^-2*eye(size(q,2)));  % = q'*q
                    end
