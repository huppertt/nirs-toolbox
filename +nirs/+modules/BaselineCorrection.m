...
%     tune - number of standard deviations to define an outlier
    ...
    properties
        ...
        PCA = false;
        modelorder='4xFs';
        post_hpf=NaN;  % add a post high-pass filter (use NaN to skip)
        verbose;
        ...
        function data = runThis( obj, data )
            ...
            for i = 1:numel(data)
                ...
                for j = 1:size(data(i).data, 2)
                    ...
                    if(isstr(obj.modelorder))
                        p = Fs*str2num(obj.modelorder(1:strfind(obj.modelorder,'x')-1));
                    else
                        p=obj.modelorder;
                    end
...
%                     [a, r] = nirs.math.robust_ar_fit(yd, round(4*Fs));
                    ...
                    [~,~,~,ymoco] = nirs.math.robust_ari1_fit(y, round(p), obj.tune);
%                     % weights
%                     s = mad(r, 0) / 0.6745;
...
%                                         
                    ...
                    if obj.verbose
                        ...
                        fprintf( 'Finished %4i of %4i.\n', j,  size(data(i).data, 2) )
                    end
                    
                    if(~isnan(obj.post_hpf))
                        j=eeg.modules.BandPassFilter;
                        j.highpass=obj.post_hpf;
                        data(i)=j.run(data(i));
                    end
