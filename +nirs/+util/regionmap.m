...
if(nargin<3)
    if(nargin<1)
        ...
        atlas={'aal' 'Brodmann (MRIcron)' 'gordan' 'mmp'};
    end
end
...
if(nargout==0)
    ...
    IM2 = griddata(xy(:,1),xy(:,2),i,X,Y,'nearest');
    IM(IM>40)=NaN;  
    IM2(isnan(IM))=NaN;
    
    h=imagesc([min(xy(:,1)):dx:max(xy(:,1))]+shiftx,[min(xy(:,2)):dy:max(xy(:,2))]+shifty,...
        IM2,[0 max(abs(IM2(:)))]);
    set(h,'alphaData',(1-nanmax((IM-nanmin(IM(:)))./(40-nanmin(IM)),0)).*(~isnan(IM)));
...
%    cm=cm(randperm(length(cm)),:);
   ...
   for i=1:length(lst)
        t(i)=text(-187,-100+12*i,alllabels.Label{lst(i)});
        p(i)=patch([-175 -185 -185 -175],[-100+12*i-5 -100+12*i-5 -100+12*i+5 -100+12*i+5],cm(i+1,:));
   end     
