...
optodes2D = table(Name, [SrcPos2D(:,1); DetPos2D(:,1); FID2D(:,1)],...
    [SrcPos2D(:,2); DetPos2D(:,2); FID2D(:,2)],...
    [SrcPos2D(:,3); DetPos2D(:,3); FID2D(:,3)],...
    Type,repmat({'mm'},cnt-1,1),...
    'VariableNames',{'Name','X','Y','Z','Type','Units'});


optodes3D = table(Name, [SrcPos3D(:,1); DetPos3D(:,1); FID3D(:,1)],...
    [SrcPos3D(:,2); DetPos3D(:,2); FID3D(:,2)],...
    [SrcPos3D(:,3); DetPos3D(:,3); FID3D(:,3)],...
    Type,repmat({'mm'},cnt-1,1),...
    'VariableNames',{'Name','X','Y','Z','Type','Units'});
