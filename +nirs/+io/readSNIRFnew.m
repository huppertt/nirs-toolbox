...
if(~isempty(snirfdata.probe.detectorPos3D))
    ...
    optodes.Type ={};
    optodes.Units={};
    for i=1:size(snirfdata.probe.sourcePos2D,1)
        lab=['000' num2str(i)];
        lab=lab(end-3:end);
    ...
    for i=1:size(snirfdata.probe.detectorPos2D,1)
        lab=['000' num2str(i)];
        lab=lab(end-3:end);
    ...
    if(isempty(snirfdata.probe.landmarkPos3D))
    ...
    if(isfield(snirfdata.metaDataTags.tags,'Model') && contains(snirfdata.metaDataTags.tags.Model,'flow'))
    ...
    probe.optodes=struct2table(optodes);
