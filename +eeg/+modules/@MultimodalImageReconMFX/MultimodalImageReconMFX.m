classdef MultimodalImageReconMFX < nirs.modules.AbstractModule
    % This is the non-linear mixed effects image reconstruction model for
    % multimodal data. 
    ...
    properties
        ...
        nlnmodel = Dictionary();  % Non-linear model Xmodality = f(X)
    end
    ...
    methods
        ...
        function G = runThis( obj,S )
            ...
            demo=table;
            for j=1:length(S)
                rescale(j)=(10*length(unique(nirs.getStimNames(S{j})))*length(S{j}));
                
                % demographics info
                demothis = nirs.createDemographicsTable( S{j} );
                if(isempty(demothis))
                    for idx=1:length(S{j});
                        S{j}(idx).demographics('subject')='default';
                    end
                end
                if(~ismember(demothis.Properties.VariableNames,'subject'));
                    for idx=1:length(S{j});
                        S{j}(idx).demographics('subject')='default';
                    end
                end
            ...
            SS=struct('modality',[],'variables',[],'beta',[],'covb',[],...
            ...
            for j=1:length(S)
                for idx=1:length(S{j})
                    ...
                    [ia,ib]=ismember(thisprobe.link,S{j}(idx).probe.link,'rows');
                    ibAll=[];
                    for i=1:length(S{j}(idx).conditions);
                        ibAll=[ibAll; ib+(length(ib)*(i-1))];
                    end
                    SS(cnt).demographics=S{j}(idx).demographics;
                    SS(cnt).conditions=S{j}(idx).conditions;
                    SS(cnt).modality=modality;
                    SS(cnt).variables=S{j}(idx).variables(ibAll,:);
                    SS(cnt).beta=S{j}(idx).beta(ibAll);
                    SS(cnt).covb=S{j}(idx).covb(ibAll,ibAll);
                    SS(cnt).probe=S{j}(idx).probe;
                    SS(cnt).probe.link=S{j}(idx).probe.link(ib,:);
                    cnt=cnt+1;
...
%             
%             for i = 1:length(SS)
%                 conds=unique(nirs.getStimNames(SS(i)));
%                 [u, s, ~] = svd(SS(i).covb, 'econ');
%                 W = diag(1./diag(sqrt(s))) * u';
%                 
%                 
%                 if obj.prior.iskey(SS(i).demographics('subject'))
%                     key = SS(i).demographics('subject');
%                 else
%                     key = 'default';
%                 end
%                 xx=[];
%                 
%                 modality=SS(i).modality;
%                 
%                 J =L([key ':' modality]);
%                 flds=fields(J);
%                 
...
%                         x2=Lfwdmodels([key ':' modality ':' flds{fIdx}])*V';
%         
%                        xlocal=[xlocal x2]; 
%                     end
%                     
...
%             end
            ...
            demo = nirs.createDemographicsTable( SS );
            ...
            for i = 1:length(SS)
                % coefs
                b = [b; SS(i).beta];
                bLst=[bLst; repmat(i,size(SS(i).beta))];
                % whitening transform
                [u, s, ~] = svd(SS(i).covb, 'econ');
                W = blkdiag(W, diag(1./diag(eps(1)+sqrt(s))) * u');
                
                % table of variables
                variables=SS(i).variables;
                conds=unique(variables.cond);
