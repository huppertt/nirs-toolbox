...
if nargin < 4,
    fileExt  = {'.vhdr','.fif','.txt', 'xdf'};
end
if nargin < 3,
    loadFunc = {@(file)eeg.io.loadBrainVision(file,100),@(file)eeg.io.loadFiff(file,100),@(file)eeg.io.loadOpenBCI(file,250),@(file)eeg.io.load_xdf_generic(file)};
end
