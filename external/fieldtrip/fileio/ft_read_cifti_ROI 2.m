...
if readdata
    ...
    n=hdr.dim(6);
    V=zeros(size(ROI,2),hdr.dim(7));
    for ii=1:hdr.dim(7)
        disp(ii);
        switch hdr.datatype
            case   2, [voxdata, nitemsread] = fread(fid, n, 'uchar'); %  assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
            case   4, [voxdata, nitemsread] = fread(fid, n, 'short'); %  assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
            case   8, [voxdata, nitemsread] = fread(fid, n, 'int');   %  assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
            case  16, [voxdata, nitemsread] = fread(fid, n, 'float'); %  assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
            case  64, [voxdata, nitemsread] = fread(fid, n, 'double'); % assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
            case 512, [voxdata, nitemsread] = fread(fid, n, 'ushort'); % assert(nitemsread==prod(hdr.dim(2:end)), 'could not read all data');
